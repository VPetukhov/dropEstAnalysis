...
gg_fig <- gg_template + 
  geom_point_rast(aes(color=MitochondrionFraction), size=0.1, alpha=0.15, width=6, height=4, dpi=200) +
  scale_color_gradientn(colours=c("#1200ba", "#347fff", "#cc4000", "#ff3333"), 
                        values=scales::rescale(c(0, 0.1, 0.3, 0.8)), 
                        breaks=seq(0, 1.0, 0.2)) +
  guides(color=guide_colorbar(direction="horizontal", title.position="top", 
  ...
  geom_segment(aes(xend=MouseEnd, yend=HumanEnd, group=Type),  data=arrows_df, arrow=arrow(length = unit(0.03, "npc"))) +
