...
b_markers_df <- data.frame(
  Type = c("Immature B cells", "Pre-pro B cells", "Pro B cells"),
...
## Erythroblasts
[Erythrocyte maturing paper](https://cosmosscholars.com/phms/index.php/gjhbt/article/viewFile/56/10)  
[KRT1 is expressed at erythroblasts](https://discovery.lifemapsc.com/in-vivo-development/blood/hematopoietic-bone-marrow/erythroblasts)  
[CD47, CD71, RHAG as markers of differentiation](https://www.ncbi.nlm.nih.gov/pmc/articles/PMC2762680/). Fig.1.  
[CD71](http://sci-hub.tw/https://link.springer.com/article/10.1007/s10059-012-0090-6)  

CD117(KIT)-, CD105(ENG)-, CD10(MME)-, CD33-, CD34-, CD38-

```{r, fig.width=8, fig.height=9, message=FALSE, warning=FALSE}
genes <- tibble(CD36="CD36", CD235a="GYPA", CD71="TFRC", KRT1="KRT1", RHAG="RHAG", 
                CD45="PTPRC", `HLA-DR`="HLA-DRB1", RPS11="RPS11")


clust_cbs <- names(clusters_annotated)[clusters_annotated == 'Erythroblasts']
clust_points <- pgd$embeddings$PCA$tSNE[clust_cbs,]

plotPagoda <- function(title, ...) 
  PlotPagodaEmbeding(pgd, size=1, alpha=0.5, raster=T, raster.width=8/3, 
                     raster.height=3, ...) + 
  ggtitle(title) +  ggrastr::theme_pdf(show.ticks=F) + 
  ylim(range(clust_points[,2])) + xlim(range(clust_points[,1])) +
  theme(axis.title.x=element_blank(), axis.title.y=element_blank())

ggs <- mapply(function(g, n) plotPagoda(n, colors=log10(1e-3 + pgd$counts[,g])) + 
                scale_color_distiller(palette="RdYlBu"), genes, names(genes), SIMPLIFY=F)
ggs <- c(list(plotPagoda("Clusters", clusters=clusters)), ggs)

gg <- cowplot::plot_grid(plotlist=ggs, ncol=3)
```
