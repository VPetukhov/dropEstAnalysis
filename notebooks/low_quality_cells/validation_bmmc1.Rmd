...
```{r}
(table(clusters_annotated_resc[rescued_cbs]) / 
  (table(clusters_annotated) + table(clusters_annotated_resc[rescued_cbs]))) %>% 
  as_tibble() %>% mutate(n = round(100 * n, 1)) %>%
  `colnames<-`(c("Cell type", "Fraction of rescued, %"))
```
...
ggsave(paste0(kPlotsFolder, 'fig_bmmc_filtration.pdf'), gg_fig, width=7.5, height=7)
