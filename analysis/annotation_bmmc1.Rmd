...
Initial clustering:  
```{r}
PlotPagodaEmbeding(pgd, clusters=clusters)
```
...
```{r}
de_genes <- pgd$getDifferentialGenes(type='PCA', clusterType='infomap', 
                                     upregulated.only=T) %>%
  lapply(rownames)
...
if (length(major_type_clusts) != length(unique(major_type_clusts))) stop("Something goes wrong")
```
